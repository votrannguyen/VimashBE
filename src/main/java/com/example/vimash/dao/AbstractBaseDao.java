...
public abstract class AbstractBaseDao {
	...
	public CustomerIPageResponse findAll(PageRequest searchDTO) {
		...
		list.stream().forEach(l -> {
			CustomerResponse lrspp = new CustomerResponse(l.getCustomerId(), l.getCustomerCode(), l.getPostCode(),
					l.getCustomerName(), l.getPicName(), l.getAddress1(), l.getPhoneNumber(), l.getFaxNumber(),
					l.getLeadTime(), l.getRouteCode(), l.getCourseCode());
			lrsp.add(lrspp);
		});
		customerIPageResponse.pageInfo(searchDTO.getCurrentPage(), searchDTO.getNoRecordInPage(),
				countTotalRecord(searchDTO));
